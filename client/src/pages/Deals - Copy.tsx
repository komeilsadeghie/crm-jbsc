...
const Deals = () => {
  ...
  const totalValue = deals?.reduce((sum: number, deal: any) => sum + (deal.budget || 0), 0) || 0;
  const weightedValue = deals?.reduce((sum: number, deal: any) => 
    sum + ((deal.budget || 0) * (deal.probability || 0) / 100), 0) || 0;

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">مدیریت پروژه‌ها (Deals)</h1>
        ...
        <div className="flex items-center gap-4">
          <div className="flex bg-gray-100 rounded-lg p-1">
            ...
            <button
              ...
              className={`px-4 py-2 rounded ${viewMode === 'funnel' ? 'bg-white shadow' : ''}`}
            >
              قیف فروش
            </button>
          </div>
      ...
      {/* Funnel View */}
      {viewMode === 'funnel' && pipeline && (
        <div className="card">
          <h2 className="text-xl font-bold mb-4">قیف فروش</h2>
        ...
        <div className="card">
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b">
                  <th className="text-right p-3">عنوان پروژه</th>
                  <th className="text-right p-3">مشتری</th>
                  <th className="text-right p-3">مرحله</th>
                  <th className="text-right p-3">بودجه</th>
                  <th className="text-right p-3">احتمال</th>
                  <th className="text-right p-3">ارزش پیش‌بینی</th>
                ...
                {deals?.map((deal: any) => {
                  ...
                  return (
                    <tr key={deal.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-3">{deal.account_name || '-'}</td>
                      <td className="p-3">
            ...
            {(!deals || deals.length === 0) && (
              <div className="text-center py-12 text-gray-500">پروژه‌ای یافت نشد</div>
            )}
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium mb-2">تاریخ تحویل پیش‌بینی شده</label>
              <input
                type="date"
